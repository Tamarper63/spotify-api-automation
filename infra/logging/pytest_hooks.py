...
def pytest_runtest_makereport(item, call):
    ...
    if report.when == "call":
        ...
        client = item.funcargs.get("spotify_user_client") or item.funcargs.get(
            "spotify_client"
        )
        if (
            client
            and hasattr(client, "request_handler")
            and hasattr(client.request_handler, "api_call_metrics")
        ):
            metrics = client.request_handler.api_call_metrics
            if metrics:
                lines = [
                    f"{m['method']} {m['url']} took {m['duration_sec']:.3f}s (status {m['status_code']})"
                    for m in metrics
                ]
