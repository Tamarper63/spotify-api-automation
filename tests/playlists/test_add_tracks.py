...
def test_add_tracks_should_return_201(spotify_user_client, default_playlist_id, sample_uris):
...
@pytest.mark.positive
def test_add_tracks_with_position_should_return_201(spotify_user_client, default_playlist_id, sample_uris):
    response = spotify_user_client.add_tracks_to_playlist(
        playlist_id=default_playlist_id,
        uris=sample_uris,
        position=0
...
def test_add_tracks_without_auth(unauthenticated_playlist_client, default_playlist_id, sample_uris):
    ...
    assert_error_response(response, expected_status_codes=400, expected_message_substring="Only valid bearer")
...
@pytest.mark.negative
def test_add_tracks_with_invalid_uri(spotify_user_client, default_playlist_id, invalid_track_uri):
    response = spotify_user_client.add_tracks_to_playlist(
        playlist_id=default_playlist_id,
