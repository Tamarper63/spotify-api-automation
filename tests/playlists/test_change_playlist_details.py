...
def new_temp_playlist(spotify_user_client):
    ...
    playlist_id = spotify_user_client.create_playlist(user_id, playlist_name).json()[
        "id"
    ]
    yield playlist_id
...
def test_public_update_succeeds_even_without_follow(
    spotify_user_client, new_temp_playlist
):
    response = spotify_user_client.change_playlist_details(
        playlist_id=new_temp_playlist, public=True
    )
    assert_status_code_ok(
        response, 200, "Spotify allows making playlist public without follow"
    )
...
def test_change_playlist_public_and_collaborative_should_return_403(
    spotify_user_client, followed_playlist
):
    response = spotify_user_client.change_playlist_details(
        playlist_id=followed_playlist, public=True, collaborative=True
    )
    assert_error_response(
        response,
...
@pytest.mark.behavior
def test_change_playlist_with_empty_name_should_not_change_name(
    spotify_user_client, followed_playlist
):
    ...
    response = spotify_user_client.change_playlist_details(
        playlist_id=followed_playlist, name=""
    )
    ...
    assert (
        updated_playlist["name"] == original_name
    ), "❌ Playlist name should not change on empty update"


@pytest.mark.negative
def test_change_playlist_with_long_description_should_return_400(
    spotify_user_client, followed_playlist
):
    long_description = "A" * 1001
    response = spotify_user_client.change_playlist_details(
        playlist_id=followed_playlist, description=long_description
    )
    assert_error_response(
...
def test_change_playlist_with_invalid_id_should_return_404(spotify_user_client):
    response = spotify_user_client.change_playlist_details(
        playlist_id="nonexistent123", name="Should Fail"
    )
    assert_error_response(
