...
def test_get_playlist_with_market_param(
    spotify_user_client, default_playlist_id, market
):
...
def test_get_playlist_with_fields_param(
    spotify_user_client, default_playlist_id, fields
):
...
def test_get_playlist_with_additional_types(spotify_user_client, default_playlist_id):
    response = spotify_user_client.get_playlist(
        default_playlist_id, additional_types="track"
    )
    assert_status_code_ok(response, 200)
...
def test_get_playlist_with_all_params(spotify_user_client, default_playlist_id):
    response = spotify_user_client.get_playlist(
        default_playlist_id, market="US", fields="name", additional_types="track"
    )
    assert_status_code_ok(response, 200)
...
def test_get_playlist_with_invalid_id_should_return_400(
...
def test_get_playlist_invalid_token(default_playlist_id):
    ...
    assert_error_response(
        ...
        expected_message_substring="Invalid access token",
    )
...
def test_get_playlist_nonexistent_playlist_should_return_404(spotify_user_client):
    ...
    assert_error_response(
        response, expected_status_codes=404, expected_message_substring="Not found"
    )
