...
from infra.models.playlist_response import PlaylistTrackResponse
...
def test_get_playlist_items_with_optional_params(user_api_clients, default_playlist_id, market, limit, offset):
    ...
    response_json = response.json()
    assert_json_field_equals(response, "limit", limit, "Limit check")
    assert "items" in response_json and isinstance(response_json["items"], list)


@pytest.mark.contract
def test_get_playlist_items_model_validation(user_api_clients, default_playlist_id):
    response = user_api_clients.playlist.get_playlist_items(default_playlist_id)
    assert_status_code_ok(response, 200, "Validate playlist items schema")

    PlaylistTrackResponse(**response.json())
