...
def test_remove_existing_tracks_should_return_200(spotify_user_client, default_playlist_id, sample_uris):
    """
    Preconditions: Ensure tracks exist in the playlist.
    Then: Remove the tracks and assert success.
    """
    ensure_tracks_exist_in_playlist(spotify_user_client, default_playlist_id, sample_uris)
...
def test_remove_tracks_without_auth_should_return_400(unauthenticated_playlist_client, default_playlist_id,
                                                      sample_uris):
    """
...
@pytest.mark.negative
def test_remove_tracks_with_invalid_uri_should_return_400(spotify_user_client, default_playlist_id, invalid_track_uri):
    """
...
@pytest.mark.negative
def test_remove_tracks_with_empty_uri_list_should_return_400(spotify_user_client, default_playlist_id):
    """
...
@pytest.mark.negative
def test_remove_tracks_with_invalid_playlist_id_should_return_400(spotify_user_client, sample_uris):
    """
    Attempt to remove tracks from a playlist using an invalid playlist ID.
    """
    response = spotify_user_client.remove_tracks_from_playlist(
        playlist_id="invalid123",
        uris=sample_uris
    )
    assert_error_response(
        response,
